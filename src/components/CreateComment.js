        ...
        <Form onSubmit={handleSubmit(submitHandler)}>
          <Row style={{marginTop: "30px"}}>
            <Col xs={12} style={{display: 'flex', alignItems: 'flex-start'}}>
              ...
              <Form.Group className="mb-3" style={{flexGrow: 1}}>
                <Form.Control defaultValue="" name="comment" placeholder="Add your comment" as="textarea" rows={5} style={{padding: "0.75rem 0.85rem"}}
                  {...register('comment')} 
                  className={`form-control ${errors.comment ? 'is-invalid' : ''}`}/>
                <Form.Text className="invalid-feedback">{errors.comment?.message}</Form.Text>
              </Form.Group>
            </Col>
          </Row>
          ...
          <Row style={{display: 'flex'}}>
            ...
            <div style={{display: 'flex', justifyContent: 'flex-end'}}>
              <Button className="pl-5" variant="primary" type="submit" style={{padding: '0.6rem 1rem'}}>Comment</Button>
            </div>
          </Row>
        </Form>
      </div>
